...
#if defined(NDEBUG)
# error "Americs cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb9a180e784076e5166aca955008ade192062fadf6963e5dc8de712c85eedd901");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Americs: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Americs Signed Message:\n";
...
{
    // Americs: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Americs: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Americs: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Americs: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Americs: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Americs: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x398a5782eeeb90e25db452e521d16f11d0a399da4919a8061ceb0b5b3c57f2d8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Twitter 05/Oct/2020 I meet Amerix";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e3479e0253ef417b83c11069d81b8990d30186717109fa2a77aa9755c74acdf4f23f24154dc3e3e46ae00566faef6f9251e21c02786b3fdf4fa75e598a35b4e8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1638887443;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087914068;

        if (fTestNet)
        {
            block.nTime    = 1638887367;
            block.nNonce   = 385970622;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xfd71b0f8b15cca58f5be542d7db1396b1dbb07ce1005c8262b43f53df39441a0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xb4, 0xd1 }; // Americs: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AmericsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AmericsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AmericsMiner(CWallet *pwallet)
{
    printf("AmericsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("americs-miner");
    ...
    try { loop {
        ...
        printf("Running AmericsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AmericsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AmericsMiner, pwallet));
}
